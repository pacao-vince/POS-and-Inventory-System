document.addEventListener("DOMContentLoaded", function () {
  ...
  function searchProducts(query) {
    if (query.length > 0) {
      fetch(`../models/search_products.php?q=${encodeURIComponent(query)}`)
          ...
          if (Array.isArray(data) && data.length > 0) {
            data.forEach((product) => {
              ...
              row.innerHTML = `
                            ...
                            <td>₱${parseFloat(product.selling_price).toFixed(
                              2
                            )}</td>
                        `;
            ...
            searchResultsPopup.style.display = "block"; // Show search result
  ...
  function highlightSearchResultRow(row) {
    Array.from(searchResultsBody.children).forEach((r) =>
  ...
  searchInput.addEventListener("keydown", function (event) {
    ...
    if (event.key === "ArrowDown") {
      ...
      if (
        ...
        selectedSearchResultRow.nextElementSibling
      ) {
      ...
      if (
        selectedSearchResultRow &&
        selectedSearchResultRow.previousElementSibling
      ) {
        selectedSearchResultRow =
          selectedSearchResultRow.previousElementSibling;
      } else {
      ...
      if (selectedSearchResultRow) {
        ...
        const productPrice = parseFloat(
          selectedSearchResultRow.cells[2].textContent.replace("₱", "")
        );
        const product = {
  ...
  function addToTable(product) {
    const existingRow = Array.from(productListBody.rows).find(
      (row) => row.cells[0].textContent === product.product_id.toString()
    );
  ...
  window.updateTotals = function () {
    ...
    document.getElementById("grandTotal").textContent = `₱${subTotal.toFixed(
      2
    )}`;
    calculateChange();
  };
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      fetch("../includes/authentication.php", {
        method: "POST",
  ...
  function validatePayment() {
    ...
    if (!payment || payment <= 0 || isNaN(payment)) {
      ...
      showAlert(
        "Payment is insufficient. Please enter a sufficient amount.",
        "danger"
      );
      return false;
    }
  ...
  confirmSaveSaleBtn.addEventListener("click", function () {
    const data = {
      ...
      cashier_username: document
        .querySelector(".cashier-profile span")
        .textContent.replace("Cashier, ", "")
        .trim(),
      products: Array.from(productListBody.querySelectorAll("tr")).map(
        (row) => ({
  ...
  function updateDateTime() {
    ...
    document.getElementById("date-time").innerText = now.toLocaleDateString(
      "en-US",
      options
    );
  }
  ...
  document.addEventListener("keydown", function (event) {
    ...
    let highlightedButtonIndex = Array.from(modalButtons).findIndex((button) =>
    ...
    if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
      if (highlightedButtonIndex === -1) {
        ...
        if (event.key === "ArrowLeft") {
          ...
          highlightedButtonIndex =
            (highlightedButtonIndex + 1) % modalButtons.length;
        }
      }
