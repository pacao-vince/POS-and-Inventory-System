document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('get_bestseller_data.php')
        .then(response => response.json())
        .then(data => {
            const bestsellerChartElement = document.getElementById('bestsellerChart');
            if (bestsellerChartElement) {
                ...
                for (let i = 0; i < data.product_name.length; i++) {
                    backgroundColors.push(`rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 255)}, 2.3)`);
                }
