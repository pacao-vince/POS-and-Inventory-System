...
function saveSale() {
    ...
    const products = Array.from(document.querySelectorAll('#productListBody tr')).map(row => ({
    ...
    const change = parseFloat(document.getElementById('changeAmount').textContent.replace('₱', ''));

    const currentDateTime = new Date();
    const transactionTime = formatDateToMySQLString(currentDateTime); // Use MySQL date format
    const cashierName = document.querySelector('.cashier-profile span').textContent.replace('Cashier, ', '').trim();
...
function printReceipt(saleData) {
    ...
    .then(result => {
        if (result.success) {
            ...
            showAlert('Failed to print receipt:', 'danger', + result.message);
        }
    })
    .catch(error => {
